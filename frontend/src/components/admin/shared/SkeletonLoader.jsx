...
const SkeletonLoader = ({ type = 'card', count = 3, className = '' }) => {
  const Pulse = ({ h = 'h-4', w = 'w-full', rounded = 'rounded' }) => (
    <div className={`${h} ${w} ${rounded} bg-surface animate-pulse`} />
  );
    ...
    return (
      <div className={`grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 ${className}`}>
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="bg-cardBg border border-borderColor rounded-2xl p-5 space-y-3">
  ...
  if (type === 'table') {
    return (
      <div className={`space-y-2 ${className}`}>
        {/* Header */}
        <div className="bg-surface rounded-xl px-4 py-3 flex gap-4">
        ...
        {/* Rows */}
        {Array.from({ length: count }).map((_, i) => (
          <div key={i} className="bg-cardBg rounded-xl px-4 py-4 flex items-center gap-4">
  ...
  if (type === 'chart') {
    return (
      <div className={`bg-cardBg border border-borderColor rounded-2xl p-5 ${className}`}>
  ...
  return <div className={`h-16 bg-surface rounded-xl animate-pulse ${className}`} />;
};
