    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-3xl bg-secondaryBg border border-white/10 rounded-3xl shadow-2xl overflow-hidden z-10 my-8 max-h-[90vh] flex flex-col"
        >
          {/* Header */}
          <div className="p-6 border-b border-white/5 flex items-center justify-between bg-cardBg/50">
            <div>
          ...
          <form onSubmit={handleSubmit} className="p-6 overflow-y-auto space-y-6 flex-1 text-xs">
            ...
            <div>
              <label className="block text-gray-400 font-bold uppercase tracking-wider mb-2 text-[11px]">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  ...
                  className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between ${
                    sellerType === 'individual_c2c'
                      ? 'bg-primary/10 border-primary shadow-yellow-glow text-white'
                      : 'bg-cardBg border-white/5 text-gray-400 hover:border-white/20'
                  }`}
                >
                ...
                <button
                  ...
                  className={`p-3.5 rounded-2xl border text-left transition-all flex flex-col justify-between ${
                    sellerType === 'business'
                      ? 'bg-accentBlue/10 border-accentBlue shadow-blue-glow text-white'
                      : 'bg-cardBg border-white/5 text-gray-400 hover:border-white/20'
                  }`}
                >
                  <div className="flex items-center justify-between">
            ...
            <div className="space-y-4">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-black/40 border border-white/10 rounded-xl px-3.5 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-primary transition-colors text-xs font-medium"
                />
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-gray-300 font-bold mb-1.5">Category</label>
                  <select
                    value={formData.category}
                    onChange={(e) => setFormData({ ...formData, category: e.target.value })}
                    className="w-full bg-black/40 border border-white/10 rounded-xl px-3.5 py-2.5 text-white focus:outline-none focus:border-primary transition-colors text-xs font-medium cursor-pointer"
                  >
                    {CATEGORIES.map((cat) => (
                      <option key={cat.id} value={cat.id} className="bg-secondaryBg">
                        {cat.name}
                      </option>
                    ))}
                  </select>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-xl pl-8 pr-3.5 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-primary transition-colors text-xs font-bold"
                    />
                  </div>
                </div>
              </div>
                ...
                <div>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-black/40 border border-white/10 rounded-xl pl-8 pr-3.5 py-2.5 text-white placeholder-gray-500 focus:outline-none focus:border-primary transition-colors text-xs font-medium"
                    />
                  </div>
                </div>
            ...
            {sellerType === 'individual_c2c' && (
              <div className="bg-primary/5 border border-primary/20 p-4 rounded-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">Item Condition</label>
                    <select
                    ...
                    >
                      <option value="Brand New / Unopened" className="bg-secondaryBg">Brand New / Unopened</option>
                      <option value="Like New" className="bg-secondaryBg">Like New (Mint, Barely Used)</option>
                      <option value="Good" className="bg-secondaryBg">Good (Minor cosmetic wear)</option>
                      <option value="Fair" className="bg-secondaryBg">Fair (Fully functional, visible wear)</option>
                      <option value="Refurbished" className="bg-secondaryBg">Refurbished</option>
                    </select>
                  </div>

                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">Usage Duration</label>
                    <input
                      type="text"
                      value={formData.usageDuration}
                      onChange={(e) => setFormData({ ...formData, usageDuration: e.target.value })}
                      placeholder="e.g. 8 months, 1 year"
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-primary text-xs"
                    />
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">Item Location (City / Area)</label>
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-black/40 border border-white/10 rounded-xl pl-8 pr-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-primary text-xs"
                      />
                    </div>
                  </div>

                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">Delivery / Handoff Mode</label>
                    <select
                      value={formData.deliveryType}
                      onChange={(e) => setFormData({ ...formData, deliveryType: e.target.value })}
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-primary text-xs"
                    >
                      <option value="Meetup & Courier" className="bg-secondaryBg">Meetup & Courier</option>
                      <option value="Local Pickup Only" className="bg-secondaryBg">Local Pickup / Meetup Only</option>
                      <option value="Courier" className="bg-secondaryBg">Courier Dispatch Only</option>
                    </select>
                  </div>
                </div>
            ...
            {sellerType === 'business' && (
              <div className="bg-accentBlue/5 border border-accentBlue/20 p-4 rounded-2xl space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">Brand Name</label>
                    <input
                      type="text"
                      value={formData.brand}
                      onChange={(e) => setFormData({ ...formData, brand: e.target.value })}
                      placeholder="e.g. Studio Terra"
                      className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-accentBlue text-xs"
                    />
                  </div>

                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">SKU / Barcode</label>
                    <input
                      type="text"
                      value={formData.sku}
                  ...
                  <div>
                    <label className="block text-gray-300 font-semibold mb-1">Warranty Policy</label>
                    <input
                      type="text"
            ...
            {/* Description */}
            <div>
              <label className="block text-gray-300 font-bold mb-1.5">Description & Key Highlights</label>
              <textarea
                rows={3}
            ...
            <div>
              ...
              <input
                ...
                placeholder="https://..."
                className="w-full bg-black/40 border border-white/10 rounded-xl px-3.5 py-2 text-white placeholder-gray-500 focus:outline-none focus:border-primary text-xs font-mono mb-2"
              />
                ...
                {SAMPLE_IMAGES.map((img) => (
                  <button
                    ...
                    className={`relative rounded-xl overflow-hidden border flex-shrink-0 w-14 h-14 transition-all ${
                      formData.image === img.url ? 'border-primary shadow-yellow-glow scale-105' : 'border-white/10 opacity-60 hover:opacity-100'
                    }`}
                  >
            ...
            <div className="flex items-center justify-end gap-3 pt-4 border-t border-white/5">
              <button
                type="button"
                onClick={onClose}
                className="px-5 py-2.5 rounded-xl font-bold bg-white/5 text-gray-300 hover:text-white hover:bg-white/10 transition-colors"
              >
                Cancel
              </button>
              <button
