    ...
    <AnimatePresence>
      <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
        ...
        <motion.div
          ...
          className="relative w-full max-w-2xl bg-secondaryBg border border-white/10 rounded-3xl shadow-2xl overflow-hidden z-10 my-8 max-h-[90vh] flex flex-col text-xs"
        >
          {/* Header */}
          <div className="p-6 border-b border-white/5 flex items-center justify-between bg-cardBg/50">
            <div>
              ...
              <p className="text-[11px] text-gray-400 mt-1">
                Placed on {order.orderDate} • Payment: <span className="text-white font-bold">{order.paymentMethod}</span> ({order.paymentStatus})
              </p>
            </div>
            <button
              onClick={onClose}
          ...
          <div className="p-6 overflow-y-auto space-y-6 flex-1">
            ...
            {!isCancelled ? (
              <div className="bg-cardBg/50 border border-white/5 p-4 rounded-2xl">
                      ...
                      <div key={step} className="flex flex-col items-center relative z-10">
                        <div
                          className={`w-7 h-7 rounded-full flex items-center justify-center font-bold text-[10px] transition-all ${
                            isPassed
                              ...
                              : 'bg-secondaryBg text-gray-400 border border-white/10'
                          }`}
                        >
                        ...
                        <span
                          className={`text-[10px] font-bold mt-1.5 ${
                            isCurrent ? 'text-primary' : isPassed ? 'text-white' : 'text-gray-500'
                          }`}
                        >
                          {step}
                        </span>
                ...
                {order.trackingNumber && order.trackingNumber !== 'CANCELLED' && (
                  <div className="mt-4 pt-3 border-t border-white/5 flex flex-wrap items-center justify-between gap-2 text-[11px]">
                    ...
                    <span className="text-gray-400">{order.deliveryEstimate}</span>
                  </div>
                )}
              </div>
            ) : (
              <div className="bg-red-500/10 border border-red-500/20 p-4 rounded-2xl flex items-center gap-3 text-red-400">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-gray-400">{order.customerNote || 'Cancelled upon request'}</p>
                </div>
              </div>
            )}
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {/* Customer Card */}
              <div className="bg-cardBg/60 border border-white/5 p-4 rounded-2xl space-y-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  />
                  <div>
                    <h4 className="text-white font-bold text-sm">{order.customer?.name}</h4>
            ...
            {/* Ordered Items List */}
            <div className="bg-cardBg/60 border border-white/5 rounded-2xl overflow-hidden">
              <div className="p-3.5 border-b border-white/5 font-extrabold text-white text-xs uppercase tracking-wider flex items-center gap-2">
                ...
                {order.items.map((item, index) => (
                  <div key={index} className="p-4 flex items-center justify-between gap-4">
                    ...
                    <div className="text-right">
                      ...
                      <span className="text-[10px] text-gray-400">₹{item.price.toLocaleString('en-IN')} / unit</span>
                    </div>
                  </div>
                ))}
              </div>
              <div className="p-4 bg-black/40 border-t border-white/5 flex justify-between items-center font-bold">
            ...
            {!isCancelled && (
              <div className="bg-cardBg/40 border border-white/5 p-4 rounded-2xl space-y-3">
                ...
                {isUpdatingTracking ? (
                  <div className="space-y-3">
                    <div className="grid grid-cols-2 gap-3">
                      <div>
                        <label className="block text-gray-400 font-bold mb-1">Carrier Name</label>
                        <input
                          type="text"
                          value={carrier}
                      ...
                      <div>
                        <label className="block text-gray-400 font-bold mb-1">Tracking AWB #</label>
                        <input
                          type="text"
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    ...
                    {order.status !== 'Delivered' && (
                      <button
                        ...
                        className="px-4 py-2.5 rounded-xl font-bold bg-red-500/10 border border-red-500/20 text-red-400 hover:bg-red-500/20 ml-auto"
                      >
                        Cancel Order
                      </button>
                    )}
          ...
          {/* Footer */}
          <div className="p-4 border-t border-white/5 bg-cardBg/40 flex justify-end">
            <button
              type="button"
              onClick={onClose}
              className="px-5 py-2 rounded-xl font-bold bg-white/5 text-gray-300 hover:text-white"
            >
              Close Window
            </button>
          </div>
