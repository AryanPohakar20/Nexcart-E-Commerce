...
const MarketplaceProduct = () => {
    ...
    return (
      <div className="min-h-screen py-32 flex flex-col items-center justify-center text-center">
        ...
        <button
          ...
          className="btn-glow-yellow text-black px-6 py-2"
        >
          Back to Marketplace
        </button>
      </div>
  ...
  const isOwnProduct =
    ...
    user?._id &&
    rawSellerId &&
    user._id.toString() === rawSellerId.toString();
          ...
          {product.images && product.images.length > 1 && (
            <div className="flex gap-3 overflow-x-auto pb-2 no-scrollbar">
              {product.images.map((img, idx) => (
                <div
                  key={idx}
                  className="w-20 h-20 shrink-0 border border-white/10 rounded-xl overflow-hidden cursor-pointer hover:border-primary transition-all"
                >
                  <img
