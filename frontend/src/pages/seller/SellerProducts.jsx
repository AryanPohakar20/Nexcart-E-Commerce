...
const SellerProducts = () => {
  ...
  return (
    <div className="space-y-6 text-left">
          ...
          <div className="flex items-center bg-black/40 border border-white/10 p-1 rounded-xl">
            <button
              ...
              className={`p-2 rounded-lg transition-colors ${
                viewMode === 'table' ? 'bg-primary text-black' : 'text-gray-400 hover:text-white'
              }`}
              title="Table View"
            >
              <FiList size={16} />
      ...
      <div className="bg-cardBg/90 backdrop-blur-sm border border-white/5 p-4 rounded-2xl space-y-4">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 text-xs">
          ...
          <div>
            ...
            >
              <option value="all" className="bg-secondaryBg">All Formats (C2C & Retail)</option>
              <option value="individual_c2c" className="bg-secondaryBg">Individual C2C Only</option>
              <option value="business" className="bg-secondaryBg">Small Business / Retail Only</option>
            </select>
          </div>
          ...
          <div>
            <select
              ...
              className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-primary text-xs cursor-pointer"
            >
              <option value="all" className="bg-secondaryBg">All Categories</option>
              {CATEGORIES.map((c) => (
                <option key={c.id} value={c.id} className="bg-secondaryBg">
                  {c.name}
                </option>
              ))}
            </select>
          ...
          <div>
            <select
              ...
              className="w-full bg-black/40 border border-white/10 rounded-xl px-3 py-2 text-white focus:outline-none focus:border-primary text-xs cursor-pointer"
            >
              <option value="all" className="bg-secondaryBg">All Statuses</option>
              <option value="active" className="bg-secondaryBg">Active (Live on Store)</option>
              <option value="draft" className="bg-secondaryBg">Draft / Paused</option>
              <option value="sold" className="bg-secondaryBg">Sold Out</option>
            </select>
          </div>
        </div>
              ...
              <tbody className="divide-y divide-white/5">
                {filteredProducts.map((p) => (
                  <tr key={p.id} className="hover:bg-white/5 transition-colors group">
                    ...
                    {/* Condition */}
                    <td className="p-4">
                      <span className="text-gray-300 font-semibold">{p.condition || 'Good'}</span>
                      {p.hasBox && <span className="block text-[9px] text-gray-500">Box + Bill</span>}
                    </td>
                    ...
                    <td className="p-4 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <button
                          ...
                          className="p-2 rounded-xl bg-white/5 text-gray-300 hover:text-primary hover:bg-white/10 transition-colors"
                          title="Edit Listing"
                        >
                          <FiEdit2 size={14} />
                        </button>
                        <button
                          ...
                          className="p-2 rounded-xl bg-white/5 text-gray-300 hover:text-accentBlue hover:bg-white/10 transition-colors"
                        ...
                        <button
                          ...
                          className="p-2 rounded-xl bg-white/5 text-gray-300 hover:text-red-400 hover:bg-red-500/10 transition-colors"
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
          {filteredProducts.map((p) => (
            <motion.div
              ...
              className="bg-cardBg/90 backdrop-blur-sm border border-white/5 hover:border-primary/30 rounded-3xl overflow-hidden p-4 space-y-3 transition-all duration-300 group flex flex-col justify-between"
            >
              ...
              <div className="pt-3 border-t border-white/5 space-y-3">
                ...
                <div className="grid grid-cols-3 gap-2">
                  <button
                    ...
                    className="py-2 rounded-xl bg-white/5 text-gray-300 hover:text-white font-bold text-[11px] flex items-center justify-center gap-1"
                  >
                    <FiEdit2 size={12} />
                    <span>Edit</span>
                  </button>
                  ...
                  <button
                    onClick={() => handleDelete(p.id, p.title)}
                    className="py-2 rounded-xl bg-red-500/10 text-red-400 hover:bg-red-500/20 font-bold text-[11px] flex items-center justify-center gap-1"
                  >
                    <FiTrash2 size={12} />
                  </button>
                </div>
